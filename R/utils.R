...
titel_search <- function(x, term) {
  ...
  if (length(split) == 1) {
    a <- grepl(split, x$Content, ignore.case = T)
  } else if (grep("ODER|oder|Oder|\\|", term, ignore.case = T) & grep("UND|und|Und|\\|", term, ignore.case = T)) {
    a <- rep(FALSE, length(x$Content))
    message("Combination of words too complex for ordering. Data is processed without ordering.")
  } else if (grep("ODER|oder|Oder|\\|", term, ignore.case = T)) {
    a <- grepl(paste(split, collapse = "|"), x$Content, ignore.case = T)
  } else if (grep("UND|und|Und|\\|", term, ignore.case = T)) {
    a <- sapply(x$Content, function(con) {
      all(sapply(split, function(z) {
        grepl(z, con, ignore.case = )
      }))
    })
  } else {
    a <- rep(FALSE, length(x$Content))
