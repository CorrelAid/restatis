...
modified_data <- function(code = "",
                          ...
                          ...){
  if(!(is.character(code)) && is.null(code)){
  ...
  if(date == "now" | identical(date, c("now", "week_before", "month_before", "year_before"))){
    date <- Sys.Date()
  }
  ...
  if( type == "all" | identical(type, c("all", "Tabellen", "Statistiken", "StatistikUpdates"))){
  ...
  splittable <- table[order(table$Date, decreasing = T), ]
  splittable <- split(splittable, splittable$Type)
  }
