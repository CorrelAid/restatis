...
library(httr2)
library(rlang)
source("//gess-fs.d.ethz.ch/home$/zkovacevic/Desktop/Ehrenamt/Correlaid/restatis/R/auth.R")
source("//gess-fs.d.ethz.ch/home$/zkovacevic/Desktop/Ehrenamt/Correlaid/restatis/R/api.R")
Sys.setenv(LANG = "de")
...
search_for <- function(term = NULL,
  ...
  if( category == "all"){
    ...
    lapply(results_json$Statistics, function(x){
      zwisch <- rbind(c("Code" = x$Code, "Content" = substr(x$Content, 1,40), "Information" = x$Information, "Cubes" = x$Cubes))
      df_stats <<- rbind(df_stats, zwisch)
    }) 
    
    df_variables <- data.frame()
    lapply(results_json$Variables, function(x){
      zwisch <- rbind(c("Code" = x$Code, "Content" = substr(x$Content, 1,40), "Type" = x$Subject, "Values" = x$Values, "Information" = x$Information))
      df_variables <<- rbind(df_variables, zwisch)
    })
    
    df_cubes <- data.frame()
    lapply(results_json$Cubes, function(x){
      zwisch <- rbind(c("Code" = x$Code, "Content" = substr(x$Content, 1,40), "Time" = x$Time, "State" = x$LatestUpdate, "State" = x$State, "Information" = x$Information))
...
search_for_valued <- function(term = NULL,
                       category = c("tables", "statistics", "variables", "cubes", "time-series"),
                       pagelength = NULL,
                       language = NULL, 
                       ...) {
  ...
  if( category == "all"){
    ...
    if(nrow(df_table) != 0){
    df_table$Titel <- grepl(term, df_table$Titel, ignore.case = T)
    ...
    if(nrow(df_stats) != 0){
    df_stats$Titel <- grepl(term, df_stats$Titel, ignore.case = T)
    ...
    df_variables$Titel <- grepl(term, df_variables$Titel, ignore.case = T)
    ...
    if(nrow(df_cubes) != 0){
    df_cubes$Titel <- grepl(term, df_cubes$Titel, ignore.case = T)
    ...
    df_time.series$Titel <- grepl(term, df_time.series$Titel, ignore.case = T)
...
aa_v <- search_for_valued("Auto")

aa_v$`Time-series`

# Spezifisch vlt weg? Oder als Zusatzinformation?