...
#' @param category Character string. Specify specific object types ('tables', 'statistics' and 'cubes'). Note that 'zensus' does not support 'cubes'. All types that are specific for one database can be used together. Default option is to use all types that are possible for the specific database.
...
#' @param pagelength Integer. Maximum length of results or objects (e.g., number of tables). Defaults to 500. Maximum of the databases is 25,000 objects. API performance, practical output limits, and speeds vary across supported databases. Adjust this parameter to optimize output performance for your target database.
  ...
  res <- lapply(database_vector, function(db) {
    ...
    if(isFALSE(debug)){
      if (all(c("tables", "statistics", "cubes") %in% category)) {
        ...
        list_resp <- list("Cubes" = if (length(list_of_cubes) == 1) {tibble::as_tibble(list_of_cubes)} else {list_of_cubes},
                          "Statistics" = if (length(list_of_stats) == 1) {tibble::as_tibble(list_of_stats)} else {list_of_stats},
                          "Tables" = if (length(list_of_tabs) == 1) {tibble::as_tibble(list_of_tabs)} else {list_of_tabs})
        ...
        if (length(list_of_stats) == 1) {
          ...
          list_resp <- list("Statistics" = list_of_stats)
        ...
        if (length(list_of_tabs) == 1) {
          ...
          list_resp <- list("Tables" = list_of_tabs)
      ...
      if (length(category) == 1 && "cubes" %in% category && db == "zensus") {
        ...
        list_resp <- list(list_of_cubes, list_of_stats, list_of_tabs)
      }
