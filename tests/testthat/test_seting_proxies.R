#-------------------------------------------------------------------------------
# Test the proxy settings functionality
#-------------------------------------------------------------------------------

test_that("Proxy settings are passed to req_proxy", {

  local_mocked_bindings(req_proxy = function(req,
                                             url,
                                             port,
                                             username,
                                             password,
  ...
  req <- httr2::request("https://example.com") %>%
    req_proxy_if_set(list(url = "localhost",
                          port = 8888,
                          username = "user",
                          password = "pass",
                          auth = "basic"))
...
test_that("Global option is picked up", {
  ...
  withr::with_options(list(restatis.proxy = list(url = "localhost",
                                                 port = 8888,
                                                 username = NULL,
                                                 password = NULL,
                                                 auth = "basic")), {

      local_mocked_bindings(req_proxy = function(req, ...) req,
                            .package = "httr2")
