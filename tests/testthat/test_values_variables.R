...
with_mock_dir("values2", {
  test_that("get_values_from_variables function returns list", {
    ...
    result <- restatis::get_values_from_variables(code = "DLAND",
                                                  detailed = TRUE,
                                                  sortcriterion = "code")
...
test_that("get_values_from_variables function errors on multiple codes", {
  expect_error(
    restatis::get_values_from_variables(code = c("611*", "711*"), detailed = TRUE, category = "tables"),
    regexp = "Parameter 'code' must be a single string.")
})

test_that("get_values_from_variables function errors on numeric code param", {
  expect_error(
    restatis::get_values_from_variables(code = 12345, detailed = TRUE, category = "tables"),
    regexp = "Parameter 'code' has to be of type 'character' or NULL.")
})

#-------------------------------------------------------------------------------

test_that("get_values_from_variables function errors on wrong sort param", {
  expect_error(
    restatis::get_values_from_variables(code = "61111", sortcriterion = "date"),
    regexp = "Parameter 'sortcriterion' has to be 'code' or 'content'.")
})

test_that("get_values_from_variables function errors on wrong sort param type", {
  expect_error(
    restatis::get_values_from_variables(code = "6111*", sortcriterion = 123),
    regexp = "Parameter 'sortcriterion' has to be of type 'character'.")
})

#-------------------------------------------------------------------------------

test_that("get_values_from_variables function errors on wrong error.ignore param", {
  expect_error(
    restatis::get_values_from_variables(code = "711*", error.ignore = 1),
