...
`{restatis}` uses abbreviations in its functions to specify the databases in
...
- At the moment, using `gen_cube()` and the latest version of `{vroom}`, there
  is a deprecation warning popping up that stems from our use of `{readr}` to
...
of `gen_auth_save`. **Important Note:** *We do only encourage this in very rare
...
respective function parameter (this overrides the credentials set by `gen_auth_save()`):
